...
From this boxplot it seems like the mean birthweight is quite consistent throughout all the categories. For the races that have more data points than others, the standard deviations are larger. Because of these observations we decided to do an ANOVA test (and it appears from the results that at least one mean is significantly different from the others, which suggests there could be a relationship between birthweight and the race of mother). It will make sense to figure out which race deviates most from others (#TODO). Currently it seems like race of mother would be a valuable predictor of birthweight.
...
Similarly from the box plot mean birthweight appears to be consistent across different categories of different sub-categories of mother's hispanic origin. Depending on the number of data points, standard deviation varies among the categories. An ANOVA test again suggests that there is at least one mean that is significantly different from the others, which means there could be some relationship with between birthweight and the type of hispanic origin of the mother. It will make sense to figure out which category deviates most from others (#TODO). Currently it seems like hispanic origin of mother would be a valuable predictor of birthweight.
