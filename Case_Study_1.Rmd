...
From this boxplot it seems like the mean birthweight is quite consistent throughout all the categories. For the races that have more data points than other, the standard deviations are larger, however from the ANOVA test (it appears that there is not a significant relationship between birthweight and race of mother??). Therefore we may not include this predictor in the final model.  
...
```
Similarly from the box plot mean birthweight appears to be consistent across different categories of different sub-categories of mother's hispanic origin. Depending on the number of data points, standard deviation varies among the categories, however from an ANOVA test ----
