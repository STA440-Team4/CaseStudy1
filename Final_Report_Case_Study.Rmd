...
model7 = glm(smoking ~ as.factor(CORES) + MAGE + as.factor(MRACER) + as.factor(MHISP_new) + as.factor(MRACER)*MAGE + as.factor(MHISP_new)*as.factor(MRACER), data = bdclean, family = binomial)
summary(model7)

ps <- predict(model7, type="response")
predict_set <- rbinom(length(ps),1,prob = ps)
